/**
 * UI for when the IVC company already exists in CRM.
 * Shows matched companies and lets the user:
 * - Add extracted management contacts to an existing match (with per-contact edit/remove)
...
interface EditableContactListProps {
  initialContacts: IvcStakeholder[];
  existingCount: number;
  totalCount: number;
  isUploading: boolean;
  onUpload: (contacts: IvcStakeholder[]) => void;
}
...
function EditableContactList({
  ...
  const [contacts, setContacts] = useState<IvcStakeholder[]>(initialContacts);
  const [editingIdx, setEditingIdx] = useState<number | null>(null);
  const [editDraft, setEditDraft] = useState<IvcStakeholder>({ name: '' });
            ...
            <div key={idx} className="bg-accent-50 border border-accent-200 rounded-xl p-3 space-y-2">
              <div className="space-y-1.5">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setEditDraft((d) => ({ ...d, email: e.target.value || undefined }))
                  }
                />
              </div>
...
export function IvcDuplicateMatch({
  ...
  const newManagement = hasManagement
    ? data.management.filter((p) => !existingNames.some((n) => namesMatch(p.name, n)))
    : [];
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="bg-white border border-warm-200 rounded-2xl overflow-hidden shadow-sm">
        ...
        {/* Contacts section (shown when a match is selected and management exists) */}
        {selectedDealId && hasManagement && (
          <>
            {isCheckingContacts ? (
            ...
            ) : newManagement.length === 0 ? (
              <div className="px-5 py-3 border-t border-warm-100 bg-warm-50">
                <p className="text-xs text-success-600">
                  All {data.management.length} contact
                  {data.management.length > 1 ? 's' : ''} already in CRM
                </p>
              </div>
            ) : (
