...
export default function App() {
  ...
  const handleConfirmedUpload = async () => {
    const toUpload = companies.filter(
      c => c.validation.valid && c.uploadStatus === 'pending' && !c.duplicate?.isDuplicate && !c.skipped
    );
  ...
  const validCount = companies.filter(c => c.validation.valid && !c.duplicate?.isDuplicate && !c.skipped).length;
          ...
          <>
            <div className="mb-4 flex items-center justify-between">
              <div className="flex gap-4 text-sm">
                ...
                {skippedCount > 0 && <span className="text-gray-500">{skippedCount} discarded</span>}
              </div>
